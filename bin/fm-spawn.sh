...
probe_pi_tui_mode() {
  ...
  if ! help=$("$executable" --help 2>&1); then
    echo "error: $adapter --help probe failed for '$executable'; refusing to launch without conclusive Pi TUI capability evidence" >&2
    return 1
  fi
  compact=$(printf '%s' "$help" | tr -d '[:space:]')
  if [ -z "$compact" ]; then
    echo "error: $adapter --help returned empty help from '$executable'; refusing to launch without conclusive Pi TUI capability evidence" >&2
    return 1
  fi
  if ! printf '%s\n' "$help" | grep -Eq '^pi - AI coding assistant([[:space:]]|$)'; then
    echo "error: $adapter --help did not return recognizable Pi help from '$executable'; refusing to launch" >&2
    return 1
  fi
  if ! printf '%s\n' "$help" | grep -Eq '^Usage:$' \
    ...
    echo "error: $adapter --help returned malformed Pi help from '$executable'; refusing to launch" >&2
    return 1
  fi
  ...
  if printf '%s\n' "$help" | grep -Eiq -- 'full[[:space:]-]*screen|alternate[[:space:]-]*(screen|tui|mode)|tui[[:space:]-]*mode'; then
    echo "error: $adapter --help advertises fullscreen or alternate-mode behavior without --tui-mode in '$executable'; refusing an ambiguous TUI launch" >&2
    return 1
  fi
  printf '%s\n' single-mode
}
...
case "$HARNESS" in
  pi|pi-signed)
    ...
    case "$PI_TUI_CAPABILITY" in
      ...
      *)
        echo "error: internal Pi TUI capability result '$PI_TUI_CAPABILITY' is invalid" >&2
        exit 1
        ;;
    esac
