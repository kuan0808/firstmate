...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written or a new process started under their agent, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A pane whose agent has started a new descendant process since its quiet window opened is deferred the same way, on the same shared cadence, because a crew blocked on one long tool call such as a multi-hour test suite renders nothing and writes nothing while the suite's own child processes keep appearing under it.
That probe walks the process table from the pane's root process (`fm_backend_pane_pid`, served by the tmux and Herdr backends; zellij, orca, and cmux serve no root) and compares only elapsed times, never a harness name or command line, so a pane whose root the backend cannot determine, or whose newest descendant predates the window, keeps the unchanged schedule.
A busy pane is otherwise exempt from staleness, but only until its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, or its `state/<id>.meta` spawn record reaches that age before any turn completes; past that bound it is routed through the same wedge escalation, with the identical reason, escalation count, worktree-write and process-tree deferral, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
...
A completed ship whose current status event is its exact `done:` report and whose canonical merge poll is armed takes that same bounded recheck as a merge wait, so a finished PR is not re-surfaced as a fresh stale wake on every pane exit, change, or watcher re-arm; any missing, custom, mismatched, expired, or revoked poll state keeps the ordinary stale path.
